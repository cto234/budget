...
const IncomeTracker = ({ incomeSources, addIncomeSource, removeIncomeSource, updateIncomeSource, totalIncomes }) => {
    return (
        <div className="max-w-md w-64 mx-auto p-4">
            <h1 className="text-2xl font-bold mb-4 bg-zinc-700 text-white p-2 rounded-lg text-center">Incomes</h1>
            {incomeSources.map((source) => (
                ...
                <div className="flex justify-center mt-4 relative group">
                    <FontAwesomeIcon icon={faPlus} onClick={addIncomeSource} className="text-3xl text-white cursor-pointer" />
